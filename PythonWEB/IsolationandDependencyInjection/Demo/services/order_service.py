...
def get_by_id(order_id: int):
    order_data = database.read_query(
...
def all(get_data_func=None):
    ...
    data = get_data_func('''SELECT id, delivery_date, delivery_address, user_id, product_id
                         FROM orders AS o
...
def get_user_orders(user: User):
    data = database.read_query('''SELECT id, delivery_date, delivery_address, user_id, product_id
                         FROM orders AS o
                           LEFT JOIN orders_products AS op
                             ON o.id = op.order_id
...
def exists(order_id: int):
    data = database.read_query('SELECT 1 from orders where id = ?', (order_id,))
...
def delete(order: Order):
    database.update_query('DELETE FROM orders_products WHERE order_id = ?', (order.id,))
    database.update_query('DELETE FROM orders WHERE id = ?', (order.id,))
